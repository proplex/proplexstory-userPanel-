...
const PhoneNumberOtp = ({ open, onOpenChange, mobileNumber, onVerify, onResend }: PhoneNumberOtpProps) => {
  const [otp, setOtp] = useState<string>("")
  const [isVerifying, setIsVerifying] = useState(false)
  const [isResending, setIsResending] = useState(false)
  const [error, setError] = useState<string>("")
  const { handleMobileOtp, error: otpError } = useMobileOtp()
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setError(error.message.message || "Failed to verify OTP. Please try again.")
    } finally {
  ...
  const handleResend = async () => {
    ...
    setError("")
    try {
      const success = await handleMobileOtp(mobileNumber, '+91')
      if (success && onResend) {
        onResend()
      }
    } catch (error: any) {
      ...
      <DialogContent className="w-[500px] max-h-[90vh] overflow-y-auto p-0">
        <DialogHeader className="bg-gray-50 p-6 space-y-2 border-b">
          <div className="flex items-center gap-2">
            ...
            <DialogTitle className="text-xl font-semibold">Verify Phone Number</DialogTitle>
          </div>
        ...
        <div className="flex flex-col px-2 py-2">
          <h2 className="text-sm font-medium text-gray-700 mb-2">Mobile Number</h2>
          <div className="bg-blue-50 rounded-lg p-3 flex items-center gap-2 border border-blue-100">
            <Phone size={16} className="text-blue-600" />
            <p className="text-gray-700 font-medium">{mobileNumber}</p>
          </div>
        </div>

        <div className="bg-gray-50 px-6 py-2 border-y">
          <div className="flex items-center justify-between gap-4">
            <p className="text-sm text-gray-600">Enter the OTP sent to your mobile number</p>
            <Button 
              variant="outline" 
              size="sm" 
        ...
        {(error || otpError) && (
          <div className="px-6 py-2">
            <p className="text-sm text-red-500">{error || otpError}</p>
          </div>
        )}
